...
#  modified by Mike Kuiper   April 2012 
#  
#  added the ability to move between frames in a trajectory while transitioning 
#  between viewpoints. 
...
proc move_vp {start end {morph_frames 50} args} {
  ...
  if {[lsearch $args "smooth_start"] > -1}  {set smooth 2}
  if {[lsearch $args "smooth_end"] > -1}    {set smooth 3}
  ...
  if {$render} {set dirName  $::VCR::dirName} ;
  if {$render} {set filePrefixName  $::VCR::filePrefixName} ;
  if {$render} {set start_frame  $::VCR::start_frame} ;
  if {$render} {set end_frame  $::VCR::end_frame} ;

  if {$render} {

#  work out trajectory frame rate change:-------------------added by MikeK----------------
#  
  ...
  set midpoint [expr ($morph_frames/2)]
  for {set j $firstj} {$j<= ($morph_frames - 1)} {incr j} {
    foreach mol [molinfo list] {
      ...
      if {$smooth == 1} {
        ...
        # accelerate smoothly at start
        set theta [expr 1.570796 * (0.0 + $j)/($morph_frames- 1)] 
        set scale_factor [expr (1 - cos($theta))]
     
      } elseif {$smooth == 3} {
        # accelerate smoothly at end
        set theta [expr 1.570796 * (0.0 - $j)/($morph_frames - 1)] 
        set scale_factor [expr (-sin($theta))]
...
## --- modified by MikeK
#      - use tachyon renderer to generate .dat files for later rendering. 
...
# update frame number: 
      ...
      set dcd_frame_int [expr (int($dcd_frame))]
      animate goto $dcd_frame_int

      display update

# generate a tachyon plot file: 
      render Tachyon [file join $dirName $::VCR::filePrefixName.$frametext.dat]   
 
      puts "Rendering frame ${dirName}/${filePrefixName}.${frametext}.plot.dat"
      puts " start: $start_frame  end:$end_frame  rate: $dcd_rate frame:$dcd_frame_int"
      incr framenum
...
proc move_vp_render {start end first_frame_num dirName filePrefixName {morph_frames 50}  {start_frame 0} {end_frame 0} args} {
  set ::VCR::first_frame_num $first_frame_num
  set ::VCR::dirName         $dirName
  set ::VCR::filePrefixName  $filePrefixName
  set ::VCR::start_frame     $start_frame
  set ::VCR::end_frame       $end_frame


#  eval move_vp $start $end $dirName  $morph_frames  -render  $args
