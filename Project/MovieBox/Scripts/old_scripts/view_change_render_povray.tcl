...
#  set start frame: --------------------------------------------------------
  ...
  for {set j 0} {$j<= ($morph_frames - 1)} {incr j} {
	...
	set framenumber    ${dirName}/${filePrefixName}.${frametext}.pov
#	set cmd_line " /usr/local/lib/vmd/tachyon_LINUXAMD64 -aasamples 16 -rescale_lights 0.36 -add_skylight 1.0 temp_plot -format PPM -o $framenumber" 
#	set cmd_line " /usr/local/lib/vmd/tachyon_LINUXAMD64 -aasamples 4  temp_plot -format PPM -o $framenumber" 
#	set cmd_line " /usr/local/lib/vmd/tachyon_LINUXAMD64 -trans_vmd  1.0 temp_plot -format PPM -o $framenumber" 

          

        set rendercmd "-Iplot.pov -O$framenumber +X +A +FP +W1280 +H720"
#	set cmd_line " povray -Iplot.pov -O$framenumber +X +A +FP +W1280 +H720" 



#       copy pov file for later batch rendering. 

#	set framenumber    ${dirName}/${filePrefixName}.${frametext}.pov
	set cmd_line " cp plot.pov $framenumber " 
	render POV3 plot.pov  
        eval exec $cmd_line 
                      
# plot Tachyon .dat file which is used in rendering process:
# render Tachyon temp_plot   
# eval exec $cmd_line

     
 puts "Rendering frame ${dirName}/${filePrefixName}.${frametext}.pov"
 incr framenum
